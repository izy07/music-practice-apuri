...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'is_completedカラムを追加しました';
  ELSE
    RAISE NOTICE 'is_completedカラムは既に存在します';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'completed_atカラムを追加しました';
  ELSE
    RAISE NOTICE 'completed_atカラムは既に存在します';
  END IF;
END $$;

-- インデックスの作成（存在しない場合のみ、カラムが存在する場合のみ）
CREATE INDEX IF NOT EXISTS idx_events_user_id ON public.events(user_id);

-- その他のインデックスの作成（カラムが存在する場合のみ）
DO $$
BEGIN
  -- dateカラムのインデックス
  IF EXISTS (
