  ...
  const getInstrumentAppeal = (nameEn: string): string => {
    if (language === 'en') {
      ...
      const appealMapJa: { [key: string]: string } = {
        ...
        Violin: 'ヴァイオリンは明るく華やかな音色を持っており、オーケストラでも主要なメロディーを奏でることが多く、中心的な役割を担います。ピアノのように減衰する音ではなく、弓がある限り音を持続できるため、音を長く伸ばしたり、音の強弱が自由自在で、音域も4オクターブ以上で広く、小さな楽器だけど大きなホールを満たす力があります。300年以上愛され続ける、永遠の名器です。',
