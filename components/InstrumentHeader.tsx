  ...
  const getInstrumentAppeal = (nameEn: string): string => {
    if (language === 'en') {
      const appealMapEn: { [key: string]: string } = {
        ...
        Other: 'Your instrument is packed with "unknown possibilities" and unique charm that don\'t fit into existing frameworks. Precisely because it\'s not a major instrument, each note you play becomes a fresh surprise for listeners and allows you to directly express your own individuality that no one else can imitate. Its one-of-a-kind resonance adds new colors to the diverse world of music and will surely leave a special mark on someone\'s heart.',
        Vocal: 'The voice is the only instrument everyone is born with—no purchase required, no carrying case. From classical and musical theatre to pop and jazz, singing connects breath, body, and emotion in a unique way. As you train your ear and support, your range and expression grow. Whether in a choir, band, or solo, the human voice can move listeners like no other instrument.',
        Conductor: 'The conductor shapes the entire ensemble without playing a single note. By reading the score in depth and communicating tempo, dynamics, and phrasing through gesture, you become the lens through which dozens of musicians play as one. It demands musical knowledge, clear technique, and the ability to inspire—a role that turns your love of music into a shared experience.',
        Euphonium: 'The euphonium fills the gap between the trumpet and the tuba with a warm, round tone. In brass bands and wind ensembles it often carries lyrical melodies and rich harmonies. Its comfortable range and singing quality make it rewarding for both beginners and advanced players. From hymn-like lines to agile solos, the euphonium brings depth and color to any ensemble.',
        Recorder: 'The recorder is one of the first instruments many people play. Its gentle, clear tone and simple fingering make it ideal for learning pitch, rhythm, and ensemble. From Renaissance consorts to modern education and early music, it has stayed relevant for centuries. Light, portable, and affordable, it opens the door to music for all ages.'
      };
      ...
      const appealMapJa: { [key: string]: string } = {
        ...
        Viola: 'ヴィオラは温かみのある中音域の響きが魅力の楽器。バイオリンより少し大きく、低い音域を持ち、オーケストラや室内楽で重要な役割を果たします。中音域の美しい音色は、まるで人の声のように語りかけるよう。和音演奏での内声部を支え、音楽に深みと温かさを加えます。ヴィオラなくして美しいハーモニーは生まれません。あなたのヴィオラが、音楽を豊かに彩ります。',
        ...
        Other: 'あなたが手にしたその楽器には、既存の枠組みには収まりきらない「未知の可能性」と独自の魅力が詰まっています。メジャーな楽器ではないからこそ、奏でられる一音一音が聴く人にとって新鮮な驚きとなり、誰にも真似できないあなただけの個性をダイレクトに表現できます。その唯一無二の響きは、多様な音楽の世界に新しい色彩を加え、誰かの心に特別な足跡を残すはずです。',
        Vocal: '声は誰もが生まれながらに持つ、唯一の楽器です。楽器を買わなくても、ケースを持ち歩かなくても、いつでも「歌」は始められます。クラシックやミュージカルからポップス・ジャズまで、歌は息と体と感情を一体にして表現する特別な手段です。耳と支えを鍛えるほど音域と表現が広がり、合唱でもバンドでもソロでも、人の声は他のどんな楽器にも代えがたい感動を届けます。',
        Conductor: '指揮者は一音も楽器を鳴らさずに、全体の音楽を形づくります。楽譜を深く読み、テンポ・強弱・フレーズを身振りで伝えることで、数十人の奏者が一つの音楽として響くように導きます。音楽の知識、明確なテクニック、そして人を動かす力が求められる、音楽愛を「みんなで共有する体験」に変える役割です。',
        Euphonium: 'ユーフォニアムは、トランペットとチューバの間を埋める、温かく丸みのある音色の金管楽器です。吹奏楽やブラスバンドでは抒情的なメロディや厚いハーモニーを担うことが多く、音域が歌いやすく、初心者から上級者まで楽しめます。賛美歌のような長いフレーズから軽やかなソロまで、アンサンブルに深みと彩りを加える存在です。',
        Recorder: 'リコーダーは多くの人が最初に手にする楽器の一つです。優しく澄んだ音色とシンプルな運指で、音程・リズム・アンサンブル感覚を学ぶのに最適。ルネサンスの合奏から現代の教育・古楽まで、何百年も愛され続けています。軽く持ち運びやすく、手頃な価格で、年齢を問わず音楽の入口を開いてくれます。'
      };
