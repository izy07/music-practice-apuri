-- 統合マイグレーション: すべてのコアテーブルを確実に作成
-- 目的: PostgREST 404エラー（テーブル未存在）を根本的に解決
-- 日付: 2026-01-30
...
-- 3. practice_sessionsテーブルの作成（完全版）
...
-- practice_sessionsテーブルのRLSポリシー（存在チェック付き）
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
-- recordingsテーブルのRLSポリシー（存在チェック付き）
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
-- 5. updated_atカラムを自動更新する関数とトリガーの作成
...
-- organizationsテーブルのRLSポリシー（存在チェック付き）
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'organizations' 
    AND policyname = 'Users can view organizations'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'organizations' 
    AND policyname = 'Users can update own organizations'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'organizations' 
    AND policyname = 'Users can delete own organizations'
  ) THEN
