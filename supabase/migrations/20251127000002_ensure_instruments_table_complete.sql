-- instrumentsテーブルの完全性を確保するマイグレーション
-- テーブルが存在しない場合は作成し、必要な楽器データをすべて投入
...
-- 4. すべての楽器データを投入（ON CONFLICTで重複を回避）
...
BEGIN
  ...
  RAISE NOTICE 'instrumentsテーブル: % 件の楽器データが存在します', instrument_count;
  IF invalid_profiles_count > 0 THEN
    RAISE WARNING '無効なinstrument_idを持つuser_profiles: % 件（NULLに設定されました）', invalid_profiles_count;
  END IF;
END $$;
