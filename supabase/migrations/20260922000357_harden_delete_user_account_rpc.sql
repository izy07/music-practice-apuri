...
-- アカウント削除 RPC の堅牢化
-- - 依存テーブルを漏れなく削除
-- - 1テーブル失敗でも続行し、最後に auth.users を削除
-- - authenticated に EXECUTE を明示付与
-- =============================================================================
...
BEGIN
  ...
  -- Storage: recordings バケット上のユーザーファイルを削除
  BEGIN
  ...
  -- 公開スキーマのユーザー紐づきテーブルを順次削除（存在しない/失敗は続行）
  ...
  LOOP
    ...
    WHEN undefined_column THEN
      -- user_id が無いテーブルは id = auth.uid() を試す（profiles系）
      BEGIN
        EXECUTE format(
  ...
  -- user_profiles は id 主キーの場合もあるため明示削除
  BEGIN
...
COMMENT ON FUNCTION public.delete_user_account() IS
  '認証済みユーザー自身のアカウントと関連データを削除する。SECURITY DEFINER。';
