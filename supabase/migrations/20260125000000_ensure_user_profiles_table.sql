-- 根本的解決: user_profilesテーブルを確実に作成
-- このマイグレーションは、テーブルが存在しない場合に確実に作成します
...
-- コメント追加（テーブルの説明）
COMMENT ON TABLE public.user_profiles IS 'ユーザープロフィール情報を格納するテーブル';
COMMENT ON COLUMN public.user_profiles.user_id IS '認証ユーザーID（auth.usersへの外部キー）';
COMMENT ON COLUMN public.user_profiles.display_name IS 'ユーザーの表示名（ニックネーム）';
COMMENT ON COLUMN public.user_profiles.selected_instrument_id IS '選択された楽器のID';
COMMENT ON COLUMN public.user_profiles.practice_level IS '練習レベル（beginner/intermediate/advanced）';
